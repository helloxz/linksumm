...
class Summ:
    ...
    async def chat(self,item:InputItem,request: Request = None):
        ...
        # 设置提示词
        prompt = f"""
你是一个内容总结助手。无论输入的是哪种语言，最后都请用中文返回。请按照以下步骤处理我提供的文本内容：

第一步：有效性判断
如果检测到输入内容明显不是网页正文（如仅包含导航栏、页脚、广告等），请友好地回复：“未检测到有效的正文内容，无法进行总结。”

第二步：过滤噪音内容
自动识别并忽略与正文无关的内容，例如菜单项、广告语、版权声明、页脚信息等。
只保留文章的核心正文内容和评论内容，确保后续总结基于有效信息。

第三步：内容总结
写出一段核心摘要 ，用最简练的语言概括全文的主要内容，控制在150字以内。
提取出关键要点列表 ，每条要点清晰简洁，突出重要信息，保持在3-5条。
若原文涉及专业领域，请用通俗易懂的语言转述，避免术语堆砌。
输出格式为：
**核心摘要：** xxx
...
**关键要点列表：**
 1. xxx
 2. xxx
请严格按照要求的格式输出（如果没有正文内容可以拒绝），不要添加额外解释或格式。
"""
